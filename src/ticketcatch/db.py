...
async def last_cheapest(session: AsyncSession, route_key: str) -> PriceQuote | None:
    ...
    then return its lowest-price row."""
    latest = await session.exec(
        select(func.max(PriceQuote.captured_at)).where(PriceQuote.route_key == route_key)
    )
    ...
    rows = await session.exec(
        select(PriceQuote)
        .where(PriceQuote.route_key == route_key, PriceQuote.captured_at == latest_at)
